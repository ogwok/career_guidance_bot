...
const MessgeBox = ({ chats }: ChatProps) => {
    return (
        <div className='flex gap-3'>
            <div className={`h-10 w-10 rounded-full ${chats.role === "user" ? "bg-blue-500" : "bg-green-500"} flex items-center justify-center text-black text-lg`}>
                {chats.role === "user" ? <FiMessageSquare /> : <TbFidgetSpinner />}
            </div>

            <div>
                <span className='font-bold mb-1'>{chats.role === "user" ? "You" : "CareerBot"}</span>
                <ReactMarkdown
